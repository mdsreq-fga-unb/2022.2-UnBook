...
function computeRank(ranks, regExpGroups, name, type, importKind, treatTypesAsGroup) {
    return (ranks[(0, import_type_1.determineImportType)({ name: name, regExpGroups: regExpGroups, importKind: importKind, treatTypesAsGroup: treatTypesAsGroup })] +
        (type === 'import' ? 0 : MAX_GROUP_SIZE));
}
function registerNode(node, name, type, ranks, regExpGroups, imported, treatTypesAsGroup) {
    var rank = computeRank(ranks, regExpGroups, name, type, node.importKind || 'value', treatTypesAsGroup);
    if (rank !== -1) {
...
var knownTypes = ['absolute', 'module', 'parent', 'sibling', 'index', 'type'];
    ...
    var rankObject = groups.reduce(function (res, group, index) {
        ...
        group.forEach(function (groupItem) {
            if (!(0, import_type_1.isRegularExpressionGroup)(groupItem) && knownTypes.indexOf(groupItem) === -1) {
                throw new Error("Incorrect configuration of the rule: Unknown type ".concat(JSON.stringify(groupItem), ". For a regular expression, wrap the string in '/', ex: '/shared/'"));
            }
...
exports.default = {
    ...
    create: function importOrderRule(context) {
        ...
        var treatTypesAsGroup = groups.includes('type');
        var alphabetize;
        var ranks;
        var regExpGroups;
        try {
            ...
            return {
                Program: function (node) {
                    context.report({ node: node, message: error.message });
                },
            };
        }
        var imported = [];
